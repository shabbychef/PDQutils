...
\providecommand{\wt}[1]{w\wrapit{#1}}
\providecommand{\fpi}[2][i]{p_{#1}\wrapit{#2}}
\providecommand{\exp}[1]{\operatorname{exp}\wrapit{#1}}


\begin{tabular}[b]{*{6}{|c|}}
class & support & $\wt{x}$ & $\fpi[i]{x}$ & $\int \wt{x}\fpi[i]{x}\fpi[j]{x}\mathrm{d}x$ & $\int \wt{x}\fpi[i]{x}\mathrm{d}x$ \\
\hline
Normal & $\wrapit{-\infty,\infty}$ & $\frac{1}{\sqrt{2\pi}}\exp{-\frac{x^2}{2}}$ & (Hermite) $He_i\wrapit{x}$ & $i! \delta_{i,j}$ & $-\frac{1}{\sqrt{2\pi}}\exp{-\frac{x^2}{2}} He_{i-1}\wrapit{x}$ \\
\hline
\end{tabular}
