...
\kdel{n,m}\knn{n},
\end{equation}
where $\kdel{m,n}$ is the Kronecker delta.  We furthermore suppose that the
...
\begin{equation}
\cdf{x} \approx \sum_{n=0}^{m} \int_{-\infty}{x} \fpi[n]{y} \wt{y}\dx[y] 
\brakit{\frac{1}{\knn{n}} \int_{-\infty}^{\infty} \fpi[n]{z} \dens{z} \dx[z]}.
\end{equation}
...
These are tabulated in Table \ref{table:orthopoly}. \cite[22.2]{abramowitz_stegun}
...
\fexp{-x}$ \\
\hline
Beta/Jacobi& $\ccint{-1,1}$ & $\funcit{f_{a+1,b+1}}{x} = \frac{1}{\Bet{a+1,b+1}2^{a+b+1}}
\wrapit{1-x}^a \wrapit{1+x}^b$ \\
\hline
\end{tabular}
...
\label{table:orthopoly_I}
\caption{orthogonal polynomials blah blah blah.}
\end{table}
